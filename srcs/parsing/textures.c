...
/*   Updated: 2025/02/12 01:36:18 by cauvray          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	handle_texture(t_game *game, char *raw_line)
{
	...
	if (!line)
		return (add_error(&game->errors, MALLOC_ERROR_MSG), false);
	if (ft_strncmp(line, NORTH_ID, 3) == 0)
	{
		if (game->north_texture)
			add_error(&game->errors,
				ft_strjoin(NORTH_ID, DUPLICATE_TEXTURE_MSG));
		else
		{
	...
	else if (ft_strncmp(line, SOUTH_ID, 3) == 0)
	{
		if (game->south_texture)
			add_error(&game->errors,
				ft_strjoin(SOUTH_ID, DUPLICATE_TEXTURE_MSG));
		else
		{
	...
	else if (ft_strncmp(line, WEST_ID, 3) == 0)
	{
		if (game->west_texture)
			add_error(&game->errors,
				ft_strjoin(WEST_ID, DUPLICATE_TEXTURE_MSG));
		else
		{
	...
	else if (ft_strncmp(line, EAST_ID, 3) == 0)
	{
		if (game->east_texture)
			add_error(&game->errors,
				ft_strjoin(EAST_ID, DUPLICATE_TEXTURE_MSG));
		else
		{
	...
	else if (ft_strncmp(line, FLOOR_ID, 2) == 0)
	{
		...
		if (!game->floor_color)
		{
			add_error(&game->errors, ft_strjoin(FLOOR_ID, INVALID_COLOR_MSG));
	...
	else if (ft_strncmp(line, CELLING_ID, 2) == 0)
	{
		...
		{
			add_error(&game->errors, ft_strjoin(CELLING_ID, INVALID_COLOR_MSG));
